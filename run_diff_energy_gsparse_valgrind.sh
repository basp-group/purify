...
echo "/* --------- RECONSTRUCT  IMAGE #SPARSITY LEVEL OF THE G MATRIX ------------*/"
 valgrind --tool=memcheck --leak-check=full --show-reachable=yes ./$CODEDIR$CODENAMER $IMAGENAME $PURIFYPATH"data/coverages/"$VISIBILITIES $PURIFYPATH$ENERGYG $ENERGYC $OUTPUTDIR $ISNR $WFRAC $RUN $FOV | tee "-a" $NEWRESDIR"RTEST_DIFF_G_"$VIS"."$IMAGENAME".W"$WFRAC".ISNR"$ISNR"/RECONSTRUCT.log"
