...
*/
  ...
  aux2 = cblas_dznrm2(Ny, (void*)y, 1);
  printf("Norm residual = %f \n\n", aux2);  

  for (i=0; i < Nx; i++){
    img_copy.pix[i] = creal(xoutc[i]);
  }  
  //purify_image_writefile(&img_copy, "data/vsa/dirty_image11.fits", filetype_img);
...
*/
  ...
  param4.verbose = 2;
  param4.max_iter = 1000;
  param4.gamma = gamma*aux2*2*sqrt(aux4);
