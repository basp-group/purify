...
*/
    ...
    //! Write image to fits file
    void write3d_header(const std::vector<Image<t_real>> &eigen_images, const pfitsio::header_params &header,
        const bool &overwrite) {
      /*
         Writes an image to a fits file.
...
*/
    ...
    std::vector<Image<t_complex>> read3d(const std::string &fits_name) {
      ...
      for (int i = 0; i < channels_total; i++) {
        Image<t_complex> eigen_image(ax1, ax2);
        std::copy(&contents[ax1 * ax2 * i], &contents[ax1 * ax2 * i] + eigen_image.size(), eigen_image.data());
