...
int main( int nargs, char const** args ){
  ...
  for (t_int cmt = 0; cmt < runs; ++cmt) {
      ...
      if (iWFRAC == -1)  {
        uv_data.w =uv_data.w *(1-resolution);
       PURIFY_INFO("W-terms: scaling  original w components ");
      }
      if (iWFRAC == 0 ){
      ...
      std::string const dirty_fits = output_filename(outputdir +inSkyName+"_sara_dirty"+std::to_string(cmt)+".fits");
      ...
      auto const padmm = sopt::algorithm::ImagingProximalADMM<t_complex>(uv_data.vis)
                         ...
                         .residual_convergence(epsilon * 1.005)
      ...
      auto dirty_image  =SimMeasurements.grid(inputData0);
      ...
      PURIFY_HIGH_LOG("[PADMM -- Thread Nbr: {} -  Gsparse: {} -  Run:{}] SNR:{}  &&  MR: {} && CT:{}",omp_get_thread_num(),energy_fraction_wproj,cmt,SNR(cmt),MR(cmt),solverTime(cmt));
  }
  return 0; 
