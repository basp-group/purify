...
int main(int, char **) {
  ...
  auto const ngc5921_2_filename = purify::notinstalled::ngc5921_2_ms();
  auto const ngc5921_2 = purify::casa::MeasurementSet(ngc5921_2_filename);

  try {
    // Gets the number of channels
    CHECK(ngc5921_2.size() == 63, "Not the number of channels I expected");
    // some of these channels are invalid
    // in which case, the different vectors are empty
    auto const channel0 = ngc5921_2[0];
    ...
    auto const channel17 = ngc5921_2[17];
    ...
    for(auto const &channel : ngc5921_2) {
    ...
    auto const filtered = ngc5921_2[std::make_pair(17, filter)];
    CHECK(filtered.lambda_w().size() == 6300, "Incorrect size for filtered data");
    // Loops can also be filtered
    for(auto i_first = ngc5921_2.begin(filter); i_first != ngc5921_2.end(filter); ++i_first) {
